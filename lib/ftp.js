 ...
 * @LastEditors: Mr.Hope
 * @LastEditTime: 2019-10-17 17:28:51
 * @Description: FTP上传
 */
    ...
    pwd().then(currentpath => {
      ...
      cwd(onlineDirectory).then(() => {
        fs.readdir(localDirectory, { withFileTypes: true }, (err2, files) => {
          if (err2) {
            console.log(`读取本地文件夹 ${localDirectory} 出错`, err2);
          ...
          Promise.all(promises).then(async () => {
            ...
            for (let i = 0; i < files.length; i++) {
              ...
              // 是文件夹
              if (file.isDirectory()) {
                console.log(`开始处理 ${localDirectory}/${file.name} 文件夹`);
                // eslint-disable-next-line no-await-in-loop
                await putFolder(`${localDirectory}/${file.name}`, `./${file.name}`);
                console.log(`${localDirectory}/${file.name} 文件夹处理完毕`);
              }
            }
